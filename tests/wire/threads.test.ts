// This file was auto-generated by Fern from our API Definition.

import * as Forum from "../../src/api/index";
import { ForumClient } from "../../src/Client";
import { mockServerPool } from "../mock-server/MockServerPool";

describe("ThreadsClient", () => {
    test("list (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                items: [
                    {
                        title: "title",
                        body: "body",
                        locked: null,
                        pinned: null,
                        extendedData: null,
                        id: "id",
                        slug: null,
                        views: 1,
                        postsCount: 1,
                        lastPostAt: null,
                        createdAt: "createdAt",
                        updatedAt: "updatedAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        };
        server.mockEndpoint().get("/threads").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.threads.list();
        expect(response).toEqual({
            data: {
                items: [
                    {
                        title: "title",
                        body: "body",
                        locked: null,
                        pinned: null,
                        extendedData: null,
                        id: "id",
                        slug: null,
                        views: 1,
                        postsCount: 1,
                        lastPostAt: null,
                        createdAt: "createdAt",
                        updatedAt: "updatedAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        });
    });

    test("list (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads").respondWith().statusCode(401).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.list();
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("list (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads").respondWith().statusCode(402).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.list();
        }).rejects.toThrow(Forum.PaymentRequiredError);
    });

    test("list (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads").respondWith().statusCode(429).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.list();
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("list (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.list();
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("create (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "title", body: "body" };
        const rawResponseBody = {
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [{ name: "name", id: "id" }],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: { key: "value" },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [{ id: "id", type: "LIKE", userId: "userId", createdAt: "2024-01-15T09:30:00Z" }],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.create({
            title: "title",
            body: "body",
        });
        expect(response).toEqual({
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [
                    {
                        name: "name",
                        id: "id",
                    },
                ],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [
                        {
                            id: "id",
                            title: "title",
                            color: null,
                            votesCount: 1.1,
                            extendedData: null,
                        },
                    ],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: {
                    key: "value",
                },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [
                    {
                        id: "id",
                        type: "LIKE",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        });
    });

    test("create (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", body: "x" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.create({
                title: "x",
                body: "x",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("create (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", body: "x" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.create({
                title: "x",
                body: "x",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("create (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", body: "x" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(402)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.create({
                title: "x",
                body: "x",
            });
        }).rejects.toThrow(Forum.PaymentRequiredError);
    });

    test("create (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", body: "x" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.create({
                title: "x",
                body: "x",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("create (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", body: "x" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.create({
                title: "x",
                body: "x",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("retrieve (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [{ name: "name", id: "id" }],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: { key: "value" },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [{ id: "id", type: "LIKE", userId: "userId", createdAt: "2024-01-15T09:30:00Z" }],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.threads.retrieve({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [
                    {
                        name: "name",
                        id: "id",
                    },
                ],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [
                        {
                            id: "id",
                            title: "title",
                            color: null,
                            votesCount: 1.1,
                            extendedData: null,
                        },
                    ],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: {
                    key: "value",
                },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [
                    {
                        id: "id",
                        type: "LIKE",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        });
    });

    test("retrieve (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(401).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrieve({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("retrieve (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(402).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrieve({
                id: "id",
            });
        }).rejects.toThrow(Forum.PaymentRequiredError);
    });

    test("retrieve (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(404).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrieve({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("retrieve (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(429).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrieve({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("retrieve (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrieve({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("delete (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.threads.delete({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("delete (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(401).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.delete({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("delete (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(402).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.delete({
                id: "id",
            });
        }).rejects.toThrow(Forum.PaymentRequiredError);
    });

    test("delete (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(404).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.delete({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("delete (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(429).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.delete({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("delete (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().delete("/threads/id").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.delete({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("update (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = {
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [{ name: "name", id: "id" }],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: { key: "value" },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [{ id: "id", type: "LIKE", userId: "userId", createdAt: "2024-01-15T09:30:00Z" }],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.update({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                title: "title",
                body: "body",
                userId: "userId",
                tags: [
                    {
                        name: "name",
                        id: "id",
                    },
                ],
                poll: {
                    id: "id",
                    title: "title",
                    closed: true,
                    closedAt: "2024-01-15T09:30:00Z",
                    expiresAt: "2024-01-15T09:30:00Z",
                    totalVotes: 1.1,
                    options: [
                        {
                            id: "id",
                            title: "title",
                            color: null,
                            votesCount: 1.1,
                            extendedData: null,
                        },
                    ],
                    userVote: "userVote",
                    createdAt: "2024-01-15T09:30:00Z",
                    updatedAt: "2024-01-15T09:30:00Z",
                },
                locked: true,
                pinned: true,
                extendedData: {
                    key: "value",
                },
                id: "id",
                slug: "slug",
                views: 1,
                postsCount: 1,
                lastPostAt: "lastPostAt",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [
                    {
                        id: "id",
                        type: "LIKE",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        });
    });

    test("update (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("update (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("update (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(402)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.PaymentRequiredError);
    });

    test("update (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("update (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("update (7)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.update({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("listPosts (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                items: [
                    {
                        id: "id",
                        threadId: "threadId",
                        userId: "userId",
                        body: "body",
                        depth: null,
                        createdAt: "createdAt",
                        updatedAt: "updatedAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        };
        server.mockEndpoint().get("/threads/id/posts").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.threads.listPosts({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                items: [
                    {
                        id: "id",
                        threadId: "threadId",
                        userId: "userId",
                        body: "body",
                        depth: null,
                        createdAt: "createdAt",
                        updatedAt: "updatedAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        });
    });

    test("listPosts (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/posts").respondWith().statusCode(401).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.listPosts({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("listPosts (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/posts").respondWith().statusCode(404).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.listPosts({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("listPosts (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/posts").respondWith().statusCode(429).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.listPosts({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("listPosts (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/posts").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.listPosts({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("retrievePost (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                id: "id",
                threadId: "threadId",
                userId: "userId",
                body: "body",
                depth: 1,
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [{ id: "id", type: "LIKE", userId: "userId", createdAt: "2024-01-15T09:30:00Z" }],
                _count: { reactions: 1.1 },
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/posts/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.retrievePost({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                threadId: "threadId",
                userId: "userId",
                body: "body",
                depth: 1,
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                reactions: [
                    {
                        id: "id",
                        type: "LIKE",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                _count: {
                    reactions: 1.1,
                },
                createdAt: "createdAt",
                updatedAt: "updatedAt",
            },
        });
    });

    test("retrievePost (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/posts/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrievePost({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("retrievePost (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/posts/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrievePost({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("retrievePost (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/posts/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrievePost({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("retrievePost (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/posts/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrievePost({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deletePostById (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/posts/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deletePostById({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deletePostById (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/posts/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePostById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deletePostById (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/posts/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePostById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deletePostById (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/posts/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePostById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deletePostById (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/posts/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePostById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("listReactions (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                items: [{ id: "id", type: "type", userId: "userId", createdAt: "createdAt" }],
                nextCursor: "nextCursor",
                count: 1,
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/reactions")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.listReactions({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                items: [
                    {
                        id: "id",
                        type: "type",
                        userId: "userId",
                        createdAt: "createdAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        });
    });

    test("listReactions (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listReactions({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("listReactions (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listReactions({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("listReactions (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listReactions({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("listReactions (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listReactions({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("createReaction (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = {
            data: {
                id: "id",
                type: "type",
                userId: "userId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.createReaction({
            id: "id",
            type: "LIKE",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                type: "type",
                userId: "userId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        });
    });

    test("createReaction (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createReaction({
                id: "id",
                type: "LIKE",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("createReaction (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createReaction({
                id: "id",
                type: "LIKE",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("createReaction (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createReaction({
                id: "id",
                type: "LIKE",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("createReaction (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createReaction({
                id: "id",
                type: "LIKE",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("createReaction (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { type: "LIKE" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/reactions")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createReaction({
                id: "id",
                type: "LIKE",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deleteReaction (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deleteReaction({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deleteReaction (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReaction({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deleteReaction (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReaction({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deleteReaction (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReaction({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deleteReaction (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReaction({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("retrieveReaction (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                id: "id",
                type: "type",
                userId: "userId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.retrieveReaction({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                type: "type",
                userId: "userId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        });
    });

    test("retrieveReaction (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveReaction({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("retrieveReaction (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveReaction({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("retrieveReaction (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveReaction({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("retrieveReaction (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveReaction({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deleteReactionById (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deleteReactionById({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deleteReactionById (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReactionById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deleteReactionById (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReactionById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deleteReactionById (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReactionById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deleteReactionById (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/reactions/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteReactionById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("listSubscribers (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                items: [{ id: "id", userId: "userId", threadId: null, createdAt: "createdAt" }],
                nextCursor: "nextCursor",
                count: 1,
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.listSubscribers({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                items: [
                    {
                        id: "id",
                        userId: "userId",
                        threadId: null,
                        createdAt: "createdAt",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        });
    });

    test("listSubscribers (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listSubscribers({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("listSubscribers (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listSubscribers({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("listSubscribers (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listSubscribers({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("listSubscribers (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listSubscribers({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("createSubscriber (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = {
            data: {
                id: "id",
                userId: "userId",
                threadId: "threadId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.createSubscriber({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                userId: "userId",
                threadId: "threadId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        });
    });

    test("createSubscriber (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createSubscriber({
                id: "id",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("createSubscriber (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createSubscriber({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("createSubscriber (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createSubscriber({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("createSubscriber (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createSubscriber({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("createSubscriber (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/subscribers")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createSubscriber({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("unsubscribe (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.unsubscribe({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("unsubscribe (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.unsubscribe({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("unsubscribe (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.unsubscribe({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("unsubscribe (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.unsubscribe({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("unsubscribe (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.unsubscribe({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("retrieveSubscriber (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                id: "id",
                userId: "userId",
                threadId: "threadId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [{ id: "id", name: "name", slug: null, color: null }],
                    extendedData: { key: "value" },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.retrieveSubscriber({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                userId: "userId",
                threadId: "threadId",
                user: {
                    id: "id",
                    username: "username",
                    displayName: "displayName",
                    bio: "bio",
                    signature: "signature",
                    url: "url",
                    postsCount: 1,
                    threadsCount: 1,
                    isOnline: true,
                    lastSeenAt: "lastSeenAt",
                    roles: [
                        {
                            id: "id",
                            name: "name",
                            slug: null,
                            color: null,
                        },
                    ],
                    extendedData: {
                        key: "value",
                    },
                    createdAt: "createdAt",
                    updatedAt: "updatedAt",
                },
                createdAt: "createdAt",
            },
        });
    });

    test("retrieveSubscriber (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveSubscriber({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("retrieveSubscriber (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveSubscriber({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("retrieveSubscriber (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveSubscriber({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("retrieveSubscriber (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.retrieveSubscriber({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deleteSubscriberById (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deleteSubscriberById({
            id: "id",
            subId: "subId",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deleteSubscriberById (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteSubscriberById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deleteSubscriberById (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteSubscriberById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deleteSubscriberById (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteSubscriberById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deleteSubscriberById (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/subscribers/subId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteSubscriberById({
                id: "id",
                subId: "subId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("retrievePoll (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        };
        server.mockEndpoint().get("/threads/id/poll").respondWith().statusCode(200).jsonBody(rawResponseBody).build();

        const response = await client.threads.retrievePoll({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [
                    {
                        id: "id",
                        title: "title",
                        color: null,
                        votesCount: 1.1,
                        extendedData: null,
                    },
                ],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        });
    });

    test("retrievePoll (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/poll").respondWith().statusCode(401).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrievePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("retrievePoll (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/poll").respondWith().statusCode(404).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrievePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("retrievePoll (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/poll").respondWith().statusCode(429).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrievePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("retrievePoll (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server.mockEndpoint().get("/threads/id/poll").respondWith().statusCode(500).jsonBody(rawResponseBody).build();

        await expect(async () => {
            return await client.threads.retrievePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("createPoll (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "title", options: [{ title: "title" }] };
        const rawResponseBody = {
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.createPoll({
            id: "id",
            title: "title",
            options: [
                {
                    title: "title",
                },
            ],
        });
        expect(response).toEqual({
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [
                    {
                        id: "id",
                        title: "title",
                        color: null,
                        votesCount: 1.1,
                        extendedData: null,
                    },
                ],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        });
    });

    test("createPoll (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", options: [{ title: "x" }, { title: "x" }] };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPoll({
                id: "id",
                title: "x",
                options: [
                    {
                        title: "x",
                    },
                    {
                        title: "x",
                    },
                ],
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("createPoll (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", options: [{ title: "x" }, { title: "x" }] };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPoll({
                id: "id",
                title: "x",
                options: [
                    {
                        title: "x",
                    },
                    {
                        title: "x",
                    },
                ],
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("createPoll (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", options: [{ title: "x" }, { title: "x" }] };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPoll({
                id: "id",
                title: "x",
                options: [
                    {
                        title: "x",
                    },
                    {
                        title: "x",
                    },
                ],
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("createPoll (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", options: [{ title: "x" }, { title: "x" }] };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPoll({
                id: "id",
                title: "x",
                options: [
                    {
                        title: "x",
                    },
                    {
                        title: "x",
                    },
                ],
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("createPoll (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { title: "x", options: [{ title: "x" }, { title: "x" }] };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPoll({
                id: "id",
                title: "x",
                options: [
                    {
                        title: "x",
                    },
                    {
                        title: "x",
                    },
                ],
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("updatePoll (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = {
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [{ id: "id", title: "title", color: null, votesCount: 1.1, extendedData: null }],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.updatePoll({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                title: "title",
                closed: true,
                closedAt: "2024-01-15T09:30:00Z",
                expiresAt: "2024-01-15T09:30:00Z",
                totalVotes: 1.1,
                options: [
                    {
                        id: "id",
                        title: "title",
                        color: null,
                        votesCount: 1.1,
                        extendedData: null,
                    },
                ],
                userVote: "userVote",
                createdAt: "2024-01-15T09:30:00Z",
                updatedAt: "2024-01-15T09:30:00Z",
            },
        });
    });

    test("updatePoll (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.updatePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("updatePoll (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.updatePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("updatePoll (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.updatePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("updatePoll (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.updatePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("updatePoll (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .patch("/threads/id/poll")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.updatePoll({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("listPollVotes (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            data: {
                items: [
                    {
                        id: "id",
                        pollId: "pollId",
                        optionId: "optionId",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        };
        server
            .mockEndpoint()
            .get("/threads/id/poll/votes")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.listPollVotes({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                items: [
                    {
                        id: "id",
                        pollId: "pollId",
                        optionId: "optionId",
                        userId: "userId",
                        createdAt: "2024-01-15T09:30:00Z",
                    },
                ],
                nextCursor: "nextCursor",
                count: 1,
            },
        });
    });

    test("listPollVotes (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/poll/votes")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listPollVotes({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("listPollVotes (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/poll/votes")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listPollVotes({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("listPollVotes (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/poll/votes")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listPollVotes({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("listPollVotes (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .get("/threads/id/poll/votes")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.listPollVotes({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("createPollVote (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = {
            data: {
                id: "id",
                pollId: "pollId",
                optionId: "optionId",
                userId: "userId",
                createdAt: "2024-01-15T09:30:00Z",
            },
        };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.createPollVote({
            id: "id",
            optionId: "optionId",
        });
        expect(response).toEqual({
            data: {
                id: "id",
                pollId: "pollId",
                optionId: "optionId",
                userId: "userId",
                createdAt: "2024-01-15T09:30:00Z",
            },
        });
    });

    test("createPollVote (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPollVote({
                id: "id",
                optionId: "optionId",
            });
        }).rejects.toThrow(Forum.BadRequestError);
    });

    test("createPollVote (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPollVote({
                id: "id",
                optionId: "optionId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("createPollVote (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPollVote({
                id: "id",
                optionId: "optionId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("createPollVote (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPollVote({
                id: "id",
                optionId: "optionId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("createPollVote (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { optionId: "optionId" };
        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .post("/threads/id/poll/votes")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.createPollVote({
                id: "id",
                optionId: "optionId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deleteOwnPollVote (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deleteOwnPollVote({
            id: "id",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deleteOwnPollVote (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteOwnPollVote({
                id: "id",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deleteOwnPollVote (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteOwnPollVote({
                id: "id",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deleteOwnPollVote (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteOwnPollVote({
                id: "id",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deleteOwnPollVote (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deleteOwnPollVote({
                id: "id",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });

    test("deletePollVote (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { data: { success: true } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes/nestedId")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.threads.deletePollVote({
            id: "id",
            nestedId: "nestedId",
        });
        expect(response).toEqual({
            data: {
                success: true,
            },
        });
    });

    test("deletePollVote (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes/nestedId")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePollVote({
                id: "id",
                nestedId: "nestedId",
            });
        }).rejects.toThrow(Forum.UnauthorizedError);
    });

    test("deletePollVote (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes/nestedId")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePollVote({
                id: "id",
                nestedId: "nestedId",
            });
        }).rejects.toThrow(Forum.NotFoundError);
    });

    test("deletePollVote (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes/nestedId")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePollVote({
                id: "id",
                nestedId: "nestedId",
            });
        }).rejects.toThrow(Forum.TooManyRequestsError);
    });

    test("deletePollVote (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new ForumClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { error: { code: "code", message: "message" } };
        server
            .mockEndpoint()
            .delete("/threads/id/poll/votes/nestedId")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.threads.deletePollVote({
                id: "id",
                nestedId: "nestedId",
            });
        }).rejects.toThrow(Forum.InternalServerError);
    });
});
